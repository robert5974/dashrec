...
Gauge::Gauge(UnitType unit_type, units_t units, QFont value_font,
  ...
  std::function<void(QByteArray)> callback =
      std::bind(&Gauge::can_callback, this, std::placeholders::_1);

  bus->registerFrameHandler(cmds[0].frame.frameId() + 0x9, callback);
  DASH_LOG(info) << "[Gauges] Registered frame handler for id "
                 << (cmds[0].frame.frameId() + 0x9);

  if (unit_type == UnitType::Distance) {
    this->si =
        (config->get_distance_unit() == Config::DistanceUnit::Kilometers);
  } else if (unit_type == UnitType::Temperature) {
    this->si =
        (config->get_temperature_unit() == Config::TemperatureUnit::Celsius);
  } else {
    this->si = true;
  }
  ...
  auto update_units = [this, units, unit_label](bool si) {
    this->si = si;
  ...
  if (unit_type == UnitType::Distance) {
    connect(config, &Config::distance_unit_changed,
            [update_units](Config::DistanceUnit unit) {
              update_units(unit == Config::DistanceUnit::Kilometers);
            });
  } else if (unit_type == UnitType::Temperature) {
    connect(config, &Config::temperature_unit_changed,
            [update_units](Config::TemperatureUnit unit) {
              update_units(unit == Config::TemperatureUnit::Celsius);
            });
  }
...
void Gauge::can_callback(QByteArray payload) {
  ...
  for (auto cmd : cmds) {
    if (cmd.frame.payload().at(2) == resp.PID) {
      value_label->setText(
          this->format_value(this->decoder(cmd.decoder(resp), this->si)));
    }
  }
}
...
QWidget *VehiclePage::dialog_body() {
  ...
  this->plugin_selector = new Selector(
      plugins, this->config->get_vehicle_plugin(),
      this->arbiter.forge().font(14), this->arbiter, widget, "unloader");

  layout->addWidget(this->distance_unit_row_widget(), 1);
  layout->addWidget(Session::Forge::br(), 1);
  layout->addWidget(this->temperature_unit_row_widget(), 1);
  layout->addWidget(Session::Forge::br(), 1);
  ...
  Selector *interface_selector = new Selector(
      devices, this->config->get_vehicle_interface(),
      this->arbiter.forge().font(14), this->arbiter, widget, "disabled");
  interface_selector->setVisible((this->can_devices.size() > 0) ||
                                 (this->serial_devices.size() > 0) ||
                                 (this->paired_bt_devices.size() > 0));
  connect(interface_selector, &Selector::item_changed, [this](QString item) {
    if (this->config->get_vehicle_can_bus() ==
            ICANBus::VehicleBusType::ELM327BT &&
        item != QString("disabled")) {
      this->config->set_vehicle_interface(this->paired_bt_devices[item]);
    } else {
      this->config->set_vehicle_interface(item);
    }
  });
  connect(this->config, &Config::vehicle_can_bus_changed,
          [this, interface_selector](int state) {
            switch (state) {
            // ELM327 USB
            case ICANBus::VehicleBusType::ELM327USB:
  ...
  connect(&this->arbiter.system().bluetooth, &Bluetooth::init,
          [this, interface_selector] {
            interface_selector->setVisible(
                (this->can_devices.size() > 0) ||
                (this->serial_devices.size() > 0) ||
                (this->paired_bt_devices.size() > 0));
            if (this->config->get_vehicle_can_bus() ==
                ICANBus::VehicleBusType::ELM327BT) {
              ...
              interface_selector->set_options(this->paired_bt_devices.keys());
              if (current != "disabled")
                interface_selector->set_current(
                    this->paired_bt_devices.key(current));
            }
          });
...
QWidget *VehiclePage::can_bus_toggle_row() {
  ...
  ICANBus::VehicleBusType can_bus_selected =
      this->config->get_vehicle_can_bus();
  QRadioButton *socketcan_button = new QRadioButton("SocketCAN", group);
  socketcan_button->setChecked(can_bus_selected ==
                               ICANBus::VehicleBusType::SocketCAN);
  ...
  elm_usb_button->setChecked(can_bus_selected ==
                             ICANBus::VehicleBusType::ELM327USB);
  ...
  elm_bt_button->setChecked(can_bus_selected ==
                            ICANBus::VehicleBusType::ELM327BT);
...
QWidget *VehiclePage::distance_unit_row_widget() {
  QWidget *widget = new QWidget(this);
  QHBoxLayout *layout = new QHBoxLayout(widget);

  QLabel *label = new QLabel("Distance", widget);
  layout->addWidget(label, 1);

  QStringList options = {"Miles", "Kilometers"};
  QString current =
      (this->config->get_distance_unit() == Config::DistanceUnit::Kilometers)
          ? "Kilometers"
          : "Miles";

  Selector *selector =
      new Selector(options, current, this->arbiter.forge().font(14),
                   this->arbiter, widget, "");
  connect(selector, &Selector::item_changed,
          [config = this->config](QString item) {
            config->set_distance_unit((item == "Kilometers")
                                          ? Config::DistanceUnit::Kilometers
                                          : Config::DistanceUnit::Miles);
          });
  layout->addWidget(selector, 1, Qt::AlignHCenter);

  return widget;
}

QWidget *VehiclePage::temperature_unit_row_widget() {
  QWidget *widget = new QWidget(this);
  QHBoxLayout *layout = new QHBoxLayout(widget);

  QLabel *label = new QLabel("Temperature", widget);
  layout->addWidget(label, 1);

  QStringList options = {"Fahrenheit", "Celsius"};
  QString current =
      (this->config->get_temperature_unit() == Config::TemperatureUnit::Celsius)
          ? "Celsius"
          : "Fahrenheit";
  ...
  Selector *selector =
      new Selector(options, current, this->arbiter.forge().font(14),
                   this->arbiter, widget, "");
  connect(
      selector, &Selector::item_changed, [config = this->config](QString item) {
        config->set_temperature_unit((item == "Celsius")
...
void VehiclePage::get_plugins() {
  for (const QFileInfo &plugin :
       Session::plugin_dir("vehicle").entryInfoList(QDir::Files)) {
...
void VehiclePage::load_plugin() {
  ...
  if (!key.isNull()) {
    ...
    if (VehiclePlugin *plugin =
            qobject_cast<VehiclePlugin *>(this->active_plugin->instance())) {
...
QWidget *DataTab::speedo_tach_widget() {
  ...
  Gauge *speed = new Gauge(
      Gauge::UnitType::Distance, {"mph", "km/h"}, speed_value_font,
      speed_unit_font, Gauge::BOTTOM, 100, {cmds.SPEED}, 0,
  ...
  Gauge *rpm = new Gauge(
      Gauge::UnitType::None, {"x1000rpm", "x1000rpm"}, tach_value_font,
...
/*  socketcan/elm327 rewrite right now only has support for one PID per gauge,
   so we can't calculate milage at this point. This is because gauges act more
   as event handlers now for each PID. Multi-PID gauges could feasibly be
   reimplemented if there was a helper method that stored received values, and
   only calls the gauge update once all values have been updated since last
   gauge update.
...
QWidget *DataTab::coolant_temp_widget() {
  ...
  Gauge *coolant_temp = new Gauge(
      Gauge::UnitType::Temperature, {"°F", "°C"}, value_font, unit_font,
      Gauge::RIGHT, 5000, {cmds.COOLANT_TEMP}, 1,
      [](double x, bool si) { return si ? x : c_to_f(x); }, widget);
...
QWidget *DataTab::engine_load_widget() {
  ...
  Gauge *engine_load = new Gauge(
      Gauge::UnitType::None, {"%", "%"}, value_font, unit_font, Gauge::RIGHT,
